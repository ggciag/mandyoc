...
PetscErrorCode _DMLocatePoints_DMDARegular_3d_2d_IS_2d(DM dm,Vec pos,IS *iscell)
{
...
PetscErrorCode DMLocatePoints_DMDARegular_3d_2d(DM dm,Vec pos,DMPointLocationType ltype, PetscSF cellSF)
{
	IS iscell;
	PetscSFNode *cells;
	...
	ierr = _DMLocatePoints_DMDARegular_3d_2d_IS_2d(dm,pos,&iscell);CHKERRQ(ierr);
...
PetscErrorCode DMGetNeighbors_DMDARegular_3d_2d(DM dm,PetscInt *nneighbors,const PetscMPIInt **neighbors)
{
	DM dmregular;
	PetscErrorCode ierr;
...
PetscErrorCode createSwarm_2d()
{
	...
	dmcell->ops->locatepoints = DMLocatePoints_DMDARegular_3d_2d;
	dmcell->ops->getneighbors = DMGetNeighbors_DMDARegular_3d_2d;
	...
	if (print_step_files==1){
		ierr = SwarmViewGP_3d_2d(dms,"step_0");CHKERRQ(ierr);
	}
