...
PetscErrorCode montaKeThermal_simplif_2d(double *Ke_local,double *Ke,double  kappa_eff);
...
double Thermal_profile_2d(double t, double zz);

PetscReal Temper3_2d(double xx,double zz);
PetscReal Temper4_2d(double xx,double zz);
...
PetscErrorCode DMDAGetElementCorners_2d(DM da,PetscInt *sx,PetscInt *sz,PetscInt *mx,PetscInt *mz)
{
	...
	ierr = DMDAGetLocalElementSize_2d(da,mx,mz);CHKERRQ(ierr);
	PetscFunctionReturn(0);
}


PetscErrorCode AssembleA_Thermal_2d(Mat A,DM thermal_da,PetscReal *TKe,PetscReal *TMe,PetscReal *TFe,
								 DM veloc_da, Vec Veloc_total)
{
	...
	ierr = DMDAGetElementCorners_2d(thermal_da,&sex,&sez,&mx,&mz);CHKERRQ(ierr);
	...
	for (ek = sez; ek < sez+mz; ek++) {
		for (ei = sex; ei < sex+mx; ei++) {
			...
			montaKeThermal_simplif_2d(TCe_fut, TKe, kappa_eff);//check: modify the simplified version
								 ...
								 DM veloc_da, Vec Veloc_total)
{
	...
	for (ek = sez; ek < sez+mz; ek++) {
		for (ei = sex; ei < sex+mx; ei++) {
			...
			montaKeThermal_simplif_2d(TCe, TKe, kappa_eff);//check: modify the simplified version
		...
		for (k=sz; k<sz+mmz; k++) {
			for (i=sx; i<sx+mmx; i++) {
				...
				if (T_initial_cond==2){
					...
					temper_aux = Thermal_profile_2d(t_inic, zz);
				}
				if (T_initial_cond==3){
					temper_aux = Temper3_2d(xx,zz);

					temper_aux-=(float)rand_r(&seed)/RAND_MAX;
				}

				if (T_initial_cond==8){
					temper_aux = Temper4_2d(xx,zz);
				}
...
PetscReal Temper3_2d(double xx,double zz){
	...
	Temper = Thermal_profile_2d(t_inic, zz);
