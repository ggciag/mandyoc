...
PetscErrorCode Swarm2Mesh3d(){
	...
	for (p=0; p<nlocal; p++) {
		...
		cy = array[3*p+1];
		if (cy>=Ly) {
			printf("Swarm2Mesh - outside: cy=%lf>=%lf\n",cy,Ly);
			cy=Ly-epsilon_x;
		}
		if (cy<=0.0) {
			printf("Swarm2Mesh - outside: cy=%lf<=0.0\n",cy);
			cy=epsilon_x;
		}
		...
		if (i<0 || i>=Nx-1) {printf("weird i=%d cx=%f , Swarm2Mesh\n",i,cx); exit(1);}
		if (j<0 || j>=Ny-1) {printf("weird j=%d cy=%f , Swarm2Mesh\n",j,cy); exit(1);}
		if (k<0 || k>=Nz-1) {printf("weird k=%d cz=%f , Swarm2Mesh\n",k,cz); exit(1);}
		...
		if (rx<0 || rx>1) {printf("weird rx=%f , Swarm2Mesh\n",rx); exit(1);}
		if (ry<0 || ry>1) {printf("weird ry=%f , Swarm2Mesh\n",ry); exit(1);}
		if (rz<0 || rz>1) {printf("weird rz=%f , Swarm2Mesh\n",rz); exit(1);}
	...
	if (visc_const_per_element==0){
		...
		if (visc_harmonic_mean==1){
			for (p=0; p<nlocal; p++) {
				...
				if (i<0 || i>=Nx-1) {printf("weird i=%d cx=%f\n",i,cx); exit(1);}
				if (j<0 || j>=Ny-1) {printf("weird j=%d cy=%f\n",j,cy); exit(1);}
				if (k<0 || k>=Nz-1) {printf("weird k=%d cz=%f\n",k,cz); exit(1);}
			...
			for (p=0; p<nlocal; p++) {
				...
				cy = array[3*p+1];
				if (cy>=Ly) {
					printf("Swarm2Mesh visc_harmonic_mean 0 - outside: cy=%lf>=%lf\n",cy,Ly);
					cy=Ly-epsilon_x;
				}
				if (cy<=0.0) {
					printf("Swarm2Mesh visc_harmonic_mean 0 - outside: cy=%lf<=0.0\n",cy);
					cy=epsilon_x;
				}
				...
				if (i<0 || i>=Nx-1) {printf("weird i=%d\n",i); exit(1);}
				if (j<0 || j>=Ny-1) {printf("weird j=%d\n",j); exit(1);}
				if (k<0 || k>=Nz-1) {printf("weird k=%d\n",k); exit(1);}
	...
	/*if (periodic_boundary==1){//!!! check for 3D!!!
		ierr = mean_value_periodic_boundary(da_Thermal,geoq_rho,local_geoq_rho,qq_rho,1);
		ierr = mean_value_periodic_boundary(da_Thermal,geoq_H,local_geoq_H,qq_H,1);
		ierr = mean_value_periodic_boundary(da_Thermal,geoq,local_geoq,qq,1);
		ierr = mean_value_periodic_boundary(da_Thermal,geoq_strain,local_geoq_strain,qq_strain,1);
	...
	for (k=sz; k<sz+mmz; k++) {
		for (j=sy; j<sy+mmy; j++) {
			for (i=sx; i<sx+mmx; i++) {
				if (qq_rho[k][j][i]<100.0){ //check: force temperature zero for low density material
					TT[k][j][i]=0.0;
				}
			}
		}
		...
		if (visc_const_per_element==1){
			...
			for (k=sz; k<sz+mmz-1; k++) {
				for (j=sy; j<sy+mmy-1; j++) {
					for (i=sx; i<sx+mmx-1; i++) {
						rho_mean = (qq_rho[k][j][i] + qq_rho[k][j][i+1] + qq_rho[k+1][j][i] + qq_rho[k+1][j][i+1])/4.0;
						if (rho_mean>rho_min && rho_mean<rho_max){
