...
for cont in range(0, 4000, 40):
    ...
    try:
        if n_cores > 1:
            tempo = np.loadtxt(
                "Tempo_" + str(cont) + ".txt", unpack=True, delimiter=":", usecols=(1)
            )
            tempo = tempo[0]
        else:
            tempo = np.loadtxt(
                "Tempo_" + str(cont) + ".txt", unpack=True, delimiter=":"
            )
    except:
    ...
    plt.title("t' = %.1f" % (tempo * 365 * 24 * 3600.0 * 1.0e-12))  # adimensional time
    # tempo in years
